...
A random forest aglorithm is trained with 100 trees for a first prediction. We can refine its parameters or compare to other algorithm later, based on its accuracy.  
```{r}
modelRF <-randomForest(classe ~ ., data = training, ntree =100)
...
```
Overall statistics: Accuracy is almost perfect (0.9998), only one row has been incorrectly classified. We can try to submit the test set.
